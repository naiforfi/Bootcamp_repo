...
We have notice that there are few missing values that can be imputed by means or median.Further more, some data need to converted to a correct data type. Also it seems that the weather column represet the weather Phenomena which is in text format. and multiple Phenomena merged together. So we will extrat  the keyword "rain" and it's synonyms and create new column tells rained or not raind. So I will work in data cleaning , Exploratory data analysis, modling and apply cross-validation to find the best model fit.
